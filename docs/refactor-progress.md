...
- [x] **P3 隧道+网络+数据源** — netx(commands/upstream/tun/latency/routing)、tunnel(logparse/command/process/openvpn，os.Pipe+Setpgid 进程组管理)、ipinfo(批量分类)、providers/vpngate(parser CSV+去重/client 回退链)。**单元测试全过**（parser、logparse 分类、BuildArgs 版本分支、upstream 解析、tun 分配）。build+vet+test 绿。约 1900 行。
- [x] **P4 服务层**（完成，~2135 行）
